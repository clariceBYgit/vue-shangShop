  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="userInfo in userAddressList"
        :key="userInfo.id"
      >
        <span class="username" :class="{ selected: userInfo.isDefault == 1 }">{{
          userInfo.consignee
        }}</span>
        <p @click="changeDefault(userInfo, userAddressList)">
          <!-- 有时候接口没有数据就是用mock数据 -->
          <!-- <span class="s1">{{userInfo.deliveryAddress}}</span>
          <span class="s2">{{userInfo.consigneeTel}}</span> -->
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getAuthTradeInfo");
  },
  methods: {
    // 点击修改默认收件人
    changeDefault(userInfo, userAddressList) {
...
// 此处 提交订单不使用 vuex
    ...
    async submitOrder() {
      ...
      let result = await this.$API.reqSubmitOrder(tradeNo,{
