      ...
      <div class="cart-body">
        ...
        >
          ...
          <li class="cart-list-con7">
            <a @click="deleteShopGood(cartInfo.skuId)" class="sindelet">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    // 删除 购物车里面的某个产品
    async deleteShopGood(skuId) {
      try {
        await this.$store.dispatch("deleteShopGood", skuId);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
    ...
    // 底部全选 按钮  将所有产品 都勾选  checkedAllCart
    async checkedAllCart(event) {
      // 当前没有 全选 按钮 对所有购物车商品进行 选中或取消的切换 接口  此时 改用 修改一个商品的状态切换接口
        try {
          let checked = event.target.checked ? "1" : "0";
          await this.$store.dispatch("updateCheckedAllCart", checked);
          this.getData();
        } catch (error) {
          alert(error.message);
        }
